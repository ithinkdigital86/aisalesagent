...
const pkceVerifierSlotKey = (storageKey, flowId)=>"".concat(storageKey, "-flow-").concat(flowId, "-code-verifier");
const pkceFlowIndexKey = (storageKey)=>"".concat(storageKey, "-flows-code-verifier");
/**
...
async function storePKCEVerifier(storage, storageKey, flowId, verifier, onEvictFlow) {
    ...
    await setItemAsync(storage, "".concat(storageKey, "-code-verifier"), verifier);
}
async function retrievePKCEVerifier(storage, storageKey, flowId) {
    ...
    const verifier = await getItemAsync(storage, "".concat(storageKey, "-code-verifier"));
    return {
        verifier: typeof verifier === 'string' ? verifier : null,
        flowId: null
    };
...
async function removePKCEVerifier(storage, storageKey, flowId) {
    const legacyKey = "".concat(storageKey, "-code-verifier");
    if (!flowId) {
...
async function removeAllPKCEVerifiers(storage, storageKey) {
    ...
    await removeItemAsync(storage, pkceFlowIndexKey(storageKey));
    await removeItemAsync(storage, "".concat(storageKey, "-code-verifier"));
}
function appendFlowIdToRedirectTo(redirectTo, flowId) {
    ...
    if (queryIndex !== -1) {
        ...
        const remaining = base.slice(queryIndex + 1).split('&').filter((pair)=>pair !== '' && pair !== __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$supabase$2f$auth$2d$js$2f$dist$2f$module$2f$lib$2f$constants$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["PKCE_FLOW_ID_PARAM"] && !pair.startsWith("".concat(__TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$supabase$2f$auth$2d$js$2f$dist$2f$module$2f$lib$2f$constants$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["PKCE_FLOW_ID_PARAM"], "=")));
        base = remaining.length > 0 ? "".concat(path, "?").concat(remaining.join('&')) : path;
    }
    const separator = base.includes('?') ? '&' : '?';
    return "".concat(base).concat(separator).concat(__TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$supabase$2f$auth$2d$js$2f$dist$2f$module$2f$lib$2f$constants$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["PKCE_FLOW_ID_PARAM"], "=").concat(encodeURIComponent(flowId)).concat(fragment);
}
async function getCodeChallengeAndMethod(storage, storageKey) {
    let isPasswordRecovery = arguments.length > 2 && arguments[2] !== void 0 ? arguments[2] : false, onEvictFlow = arguments.length > 3 ? arguments[3] : void 0;
...
class GoTrueClient {
    ...
    async linkIdentityOAuth(credentials) {
        ...
        try {
            const { data, error } = await this._useSession(async (result)=>{
                ...
                const { data, error } = result;
                if (error) throw error;
                const { url, flowId: urlFlowId } = await this._getUrlForProvider("".concat(this.url, "/user/identities/authorize"), credentials.provider, {
                    redirectTo: (_a = credentials.options) === null || _a === void 0 ? void 0 : _a.redirectTo,
                    scopes: (_b = credentials.options) === null || _b === void 0 ? void 0 : _b.scopes,
                    queryParams: (_c = credentials.options) === null || _c === void 0 ? void 0 : _c.queryParams,
    ...
    async _handleProviderSignIn(provider, options) {
        const { url, flowId } = await this._getUrlForProvider("".concat(this.url, "/authorize"), provider, {
            redirectTo: options.redirectTo,
     ...
     */ async _getUrlForProvider(url, provider, options) {
        ...
        if (redirectTo) {
            urlParams.push("redirect_to=".concat(encodeURIComponent(redirectTo)));
        }
        ...
        return {
            url: "".concat(url, "?").concat(urlParams.join('&')),
            flowId
        };
    }
    /**
     ...
     */ async _getCodeChallengeAndMethod() {
        let isPasswordRecovery = arguments.length > 0 && arguments[0] !== void 0 ? arguments[0] : false;
